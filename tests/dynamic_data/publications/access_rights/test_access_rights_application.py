...
ENDPOINTS_TO_TEST = {
    process_client.LAYER_TYPE: [
        (process_client.get_publication, {}),
        (process_client.get_publication_metadata_comparison, {}),
        (process_client.get_layer_style, {}),
        (process_client.get_publication_thumbnail, {}),
        # process_client.get_workspace_layer_chunk,
        (process_client.patch_publication, {'title': 'New title'}),
        (process_client.patch_publication, {'file_paths': ['sample/layman.layer/small_layer.geojson']}),
    ],
    process_client.MAP_TYPE: [
        (process_client.get_publication, {}),
        (process_client.get_map_file, {}),
        (process_client.get_publication_metadata_comparison, {}),
        (process_client.get_publication_thumbnail, {}),
        (process_client.patch_publication, {'title': 'New title'}),
        (process_client.patch_publication, {'file_paths': ['sample/layman.map/small_map.json']}),
    ],
}

ENDPOINTS_TO_TEST_NEGATIVE_ONLY = [
    (process_client.delete_publication, {}),
]
...
def add_publication_test_cases_to_list(tc_list, publication, user, endpoints_to_test):
    ...
    for method, args in endpoints_to_test:
        ...
        test_type = EnumTestTypes.MANDATORY if user in {
            READER_BY_USERNAME,
            READER_BY_ROLE} and method in {process_client.get_publication,
...
class TestAccessRights:
    ...
    def class_fixture(self, request):
        ...
        if request.node.session.testsfailed == 0 and not request.config.option.nocleanup:
            for publication, _, deleter in self.PUBLICATIONS_DEFS:
                process_client.delete_publication(publication.type, publication.uuid,
                                                  actor_name=deleter, )
    ...
    def test_wfst_positive(self, publication, rest_method, rest_args, params):
        ...
        rest_method(**rest_args)
        process_client.wait_for_publication_status(publication.uuid, publication.type, headers=params['headers'])
