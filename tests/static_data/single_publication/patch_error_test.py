...
from layman import LaymanError, app, util as layman_util
...
def test_patch_raster_qml(workspace, publ_type, publication):
    ...
    with pytest.raises(LaymanError) as exc_info:
        process_client.patch_layer(uuid, style_file='sample/style/ne_10m_admin_0_countries.qml', headers=headers)
...
def test_patch_qml_raster(workspace, publ_type, publication):
    ...
    with pytest.raises(LaymanError) as exc_info:
        process_client.patch_layer(uuid, file_paths=['sample/layman.layer/sample_tif_rgb.tif', ])
    test_util.assert_error(expected_exc, exc_info)
