...
def get_map_with_x_forwarded_prefix(headers, publ_uuid, publication):
    proxy_items = X_FORWARDED_ITEMS
    headers = {
        **(headers or {}),
        **proxy_items.headers,
    }
    rest_map_info = process_client.get_map(uuid=publ_uuid, headers=headers)
    exp_resp = get_expected_urls_in_rest_response(publication.workspace, process_client.MAP_TYPE, rest_map_info['name'],
                                                  rest_method='get',
                                                  x_forwarded_items=proxy_items,
                                                  geodata_type=None,
                                                  uuid=publ_uuid,
