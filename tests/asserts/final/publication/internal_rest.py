...
def same_title_and_wfs_wms_status_in_source_and_rest_multi(workspace, publ_type, name, headers):
    with app.app_context():
        publ_info = layman_util.get_publication_info(workspace, publ_type, name, context={'keys': ['title', 'wfs_wms_status']})
    ...
    if publ_type == process_client.LAYER_TYPE:
        ...
        assert info['wfs_wms_status'] == wfs_wms_status.value, f'publication_infos={publication_infos}'
    else:
        assert 'wfs_wms_status' not in info, f'publication_infos={publication_infos}'
