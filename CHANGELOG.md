...
- Add new environment variable [`LAYMAN_GS_PROXY_PORT`](doc/env-settings.md#layman_gs_proxy_port) with port part of `LAYMAN_GS_PROXY_BASE_URL`. For example `8000`.
- Stop using environment variable `LAYMAN_GS_PROXY_BASE_URL`, it has no effect to Layman anymore. Value is derived from variable `LAYMAN_PROXY_SERVER_NAME`, `LAYMAN_GS_PROXY_PORT` and `LAYMAN_GS_PATH`.
