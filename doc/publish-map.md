...
Remember that Layman supports only `EPSG:4326`, `EPSG:3857`, `EPSG:5514`, `EPSG:32633` and `EPSG:32634` projections by default for publishing layers. Therefore compositions that use WMS layers provided by Layman must use one of supported projections, otherwise these layers can not be displayed.
