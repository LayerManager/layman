...
For each raster layer, **[Coverage Store](https://docs-archive.geoserver.org/2.26.x/en/user/rest/api/coveragestores.html)**, **[Coverage](https://docs-archive.geoserver.org/2.26.x/en/user/rest/api/coverages.html)**, and **[Style](https://docs-archive.geoserver.org/2.26.x/en/user/styling/webadmin/index.html)** are created in WMS workspace. If layer uses `image_mosaic`, Coverage Store is [ImageMosaic](https://docs-archive.geoserver.org/2.26.x/en/user/data/raster/imagemosaic/index.html), otherwise it is [GeoTIFF](https://docs-archive.geoserver.org/2.26.x/en/user/data/raster/geotiff.html). [Timeseries](models.md#timeseries) layer is a specific case of ImageMosaic layer with time dimension. Names of Coverage is `l_<uuid>`, Style name is `<uuid>` name of Coverage Store is `geotiff_<uuid>` or `image_mosaic_<uuid>` depending on its type. Coverage Store and Coverage points to appropriate normalized raster GeoTIFF file(s). Style contains visualization file.
