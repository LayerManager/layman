    ...
    if muuid in csw.records:
        return {
            'metadata': {
                ...
                'comparison_url': url_for('rest_map_metadata_comparison.get', uuid=publication.uuid,
...
def get_template_path_and_values(publication: Map, *, http_method=None, actor_name):
    ...
    prop_values = _get_property_values(
        ...
        identifier_label=publication.uuid,
        extent=extent,
        crs_list=[crs],
...
def _get_property_values(
    ...
    result = {
        'md_file_identifier': MickaIds(uuid=uuid).id,
        'md_language': md_language,
