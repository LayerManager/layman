...
def patch_map(workspace, mapname, metadata_properties_to_refresh=None, actor_name=None, create_if_not_exists=True, timeout=5):
    ...
    _, prop_values = get_template_path_and_values(workspace, mapname, http_method=consts.REQUEST_METHOD_PATCH, actor_name=actor_name)
    prop_values = {
...
def csw_insert(username, mapname, actor_name):
    template_path, prop_values = get_template_path_and_values(username, mapname, http_method=consts.REQUEST_METHOD_POST, actor_name=actor_name)
