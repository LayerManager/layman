...
def refresh_csw(self, workspace, mapname, http_method=common.REQUEST_METHOD_POST, metadata_properties_to_refresh=None, actor_name=None):
    if self.is_aborted():
        raise AbortedException
    if http_method == common.REQUEST_METHOD_POST:
        csw.csw_insert(workspace, mapname, actor_name=actor_name)
    else:
        csw.patch_map(workspace, mapname, metadata_properties_to_refresh=metadata_properties_to_refresh, actor_name=actor_name)
