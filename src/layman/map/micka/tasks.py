...
from layman.common import empty_method_returns_true, consts
...
def refresh_csw(self, username, mapname, http_method=consts.REQUEST_METHOD_POST, metadata_properties_to_refresh=None, actor_name=None):
    if self.is_aborted():
...
def refresh_soap(self, username, mapname, http_method=consts.REQUEST_METHOD_POST, metadata_properties_to_refresh=None, actor_name=None, access_rights=None):
    if self.is_aborted():
        raise AbortedException
    if http_method == consts.REQUEST_METHOD_POST:
