...
def test_post_maps_simple():
    ...
    incomplete_get_resp = process_client.get_map(uuid=uuid_str)
    ...
    after_thumbnail_get_resp = process_client.get_map(uuid=uuid_str)
    ...
    after_metadata_get_resp = process_client.get_map(uuid=uuid_str)
...
def test_post_maps_complex():
    ...
    with pytest.raises(LaymanError) as exc_info:
        process_client.patch_map(uuid=uuid_str, check_response_fn=empty_method_returns_true, raise_if_not_complete=False)
    assert exc_info.value.http_code == 400
...
def test_patch_map():
    ...
    mapname = MAPNAME_1
    map_uuid = MAP_UUID_1
    patch_response = process_client.patch_map(
        uuid=map_uuid,
    ...
    incomplete_get_resp = process_client.get_map(uuid=map_uuid)
    ...
    after_thumbnail_get_resp = process_client.get_map(uuid=map_uuid)
    thumbnail = after_thumbnail_get_resp['thumbnail']
    assert 'status' not in thumbnail
    assert 'path' in thumbnail
    with app.app_context():
    ...
    get_resp_after_title = process_client.get_map(uuid=map_uuid)
    ...
    get_resp_after_description = process_client.get_map(uuid=map_uuid)
...
def test_delete_map():
    map_uuid = MAP_UUID_1
    process_client.delete_map(map_uuid)
    publication_counter.decrease()
