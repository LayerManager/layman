...
def solve_locks(workspace, publication_type, publication_name, error_code, method):
    ...
    if current_lock is None:
        return
    if method not in [common.PUBLICATION_LOCK_PATCH, common.PUBLICATION_LOCK_DELETE,
    ...
    if current_lock in [common.PUBLICATION_LOCK_PATCH, common.PUBLICATION_LOCK_POST, ]:
        if method in [common.PUBLICATION_LOCK_PATCH, common.PUBLICATION_LOCK_POST, ]:
            raise LaymanError(error_code)
    elif current_lock in [common.PUBLICATION_LOCK_DELETE, ]:
        if method in [common.PUBLICATION_LOCK_PATCH, common.PUBLICATION_LOCK_POST, ]:
            raise LaymanError(error_code)
    if method not in [common.PUBLICATION_LOCK_DELETE, ]:
        if (current_lock, method) == (common.PUBLICATION_LOCK_WFST, common.PUBLICATION_LOCK_WFST):
            ...
            assert current_lock not in [common.PUBLICATION_LOCK_WFST, ] and method not in [
                common.PUBLICATION_LOCK_WFST, ],\
