...
def setup_patch_access_rights(request_form, kwargs):
    for type in ['read', 'write']:
        if request_form.get('access_rights.' + type):
            kwargs['access_rights'] = kwargs.get('access_rights', {})
...
def get_publications(publication_type, actor, request_args=None, workspace=None, *, x_forwarded_items=None):
    ...
    for (res_workspace, _, name), info in publication_infos_whole['items'].items():
        rest_info = {
            ...
            'url': layman_util.get_publication_url(info['type'], info["uuid"], x_forwarded_items=x_forwarded_items),
            'uuid': info["uuid"],
