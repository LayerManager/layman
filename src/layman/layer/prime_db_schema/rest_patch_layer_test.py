...
def test_patch_gs_access_rights(liferay_mock):
    ...
    read_rule = geoserver.get_pattern_security_roles(f'{username}.{layername}.r', settings.LAYMAN_GS_AUTH)
    assert read_rule == {gs_username, gs_anonymous}
    write_rule = geoserver.get_pattern_security_roles(f'{username}.{layername}.w', settings.LAYMAN_GS_AUTH)
