...
def test_get_layer_title():
    ...
    with app.app_context():
        url = url_for('rest_layers.get')
    response = requests.get(url, params={'workspace': workspace}, timeout=settings.DEFAULT_CONNECTION_TIMEOUT)
...
def test_style_correctly_saved(source_style_file_path,
    ...
    with app.app_context():
        info = layer_util.get_layer_info(Layer(layer_tuple=(workspace, layer), load=False))
