...
def test_patch_current_user_without_username():
    ...
    test2_authn_headers = process_client.get_authz_headers('test2')
    test3_authn_headers = process_client.get_authz_headers('test3')

    # reserve username
    with app.app_context():
        rest_path = url_for('rest_current_user.patch', adjust_username='true')
    r = requests.patch(rest_path, headers=test2_authn_headers)
    assert r.status_code == 200, r.text

    # check if it was reserved
    with app.app_context():
        rest_path = url_for('rest_current_user.get')
    r = requests.get(rest_path, headers=test2_authn_headers)
    assert r.status_code == 200, r.text
    resp_json = r.json()
    assert resp_json['authenticated'] is True
    ...
    # TODO I guess we don't need groups anymore, consult with Franta
    # access_rights = resp_json['groups']
    # assert {'guest'} == set(access_rights.keys())
    # assert access_rights['guest'] == 'w'

    process_client.delete_map(workspace, mapname, headers=test2_authn_headers)
